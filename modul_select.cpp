    ...
    , current_modul_id(modul_id)
{
    ...
    QString db_ex = ("CREATE TABLE IF NOT EXISTS module_words_%1 (modul_id INTEGER, russian_word TEXT NO NULL, english_word TEXT NO NULL, word_id INTEGER PRIMARY KEY, FOREIGN KEY(modul_id) REFERENCES ModulList(modul_id))");
...
void modul_select::on_pushButton_2_clicked() //удаление
{
    ...
    //     qDebug() << "Не выбрана строка для удаления!";
    // }
    ...
    QString database_name = "module_words_%1"; //Имя дб с которой мы работаем
    database_name = database_name.arg(num_of_mId);
    updateQuery.prepare("UPDATE " + database_name + " SET word_id = word_id - 1 WHERE word_id > ?");
